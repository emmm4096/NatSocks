...
func main() {
	...
	case "-listen":
		if argc != 4 {
			log.Fatalln(`-listen need two arguments, like "NatSocks -listen 1997 2017".`)
		}
	...
	case "-tran":
		if argc != 4 {
			log.Fatalln(`-tran need two arguments, like "NatSocks -tran 1997 192.168.1.2:3389".`)
		}
	...
	case "-slave":
		if argc != 4 {
			log.Fatalln(`-slave need two arguments, like "NatSocks -slave 127.0.0.1:3389 8.8.8.8:1997".`)
		}
	...
	case "-resocks":
		if argc != 4 {
			log.Fatalln(`-resocks need two arguments, like "NatSocks -resocks 127.0.0.1:1080 8.8.8.8:1997".`)
		}
		var address1, address2 string
		...
		log.Println("[√]", "start socks5 server:", address1)
		go start_socks(address1)
		log.Println("[√]", "start to connect address:", address1, "and address:", address2)
		go host2host(address1, address2)
		var input string
		fmt.Scanln(&input)
		break
		case "-socks":
			if argc != 3 {
				log.Fatalln(`-socks need socks5 ip:port, like "NatSocks -socks 127.0.0.1:1080".`)
			}
			var address1 string
...
func printHelp() {
	fmt.Println(`
	 ...
	 /  \/ / _`+"`"+` | __\ \ / _ \ / __| |/ / __|
	/ /\  / (_| | |__\ \ (_) | (__|   <\__ \
	\_\ \/ \__,_|\__\__/\___/ \___|_|\_\___/  v1.0           
                                                                 -Lz1y
		`)
	fmt.Println(`
Usage: "-listen port1 port2" 			example: "NatSocks -listen 1997 2017"
       "-tran port1 ip:port2"			example: "NatSocks -tran 1997 192.168.1.2:3389" 
       "-slave ip1:port1 ip2:port2" 		example: "NatSocks -slave 127.0.0.1:3389 8.8.8.8:1997" 
       "-resocks ip1:port1 ip2:port2" 		example: "NatSocks -resocks 127.0.0.1:1080 8.8.8.8:1997" 
       "-socks ip:port" 			example: "NatSocks -socks 127.0.0.1:1080"`)
